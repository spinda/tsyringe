...
test("resolves dependencies of superclass with no constructor", async () => {
  class Dependency {}
  @injectable()
  class SuperClass {
  ...
  expect((await globalContainer.resolve(SubClass)).dependency).toBeInstanceOf(
    Dependency
  );
});
// --- resolveAll() ---
...
test("clears cached instances from container.resolve() calls", async () => {
  @singleton()
  class Foo {}
  const instance1 = await globalContainer.resolve(Foo);
...
test("instancePerContainerCachingFactory caches the returned instance", async () => {
  const factory = instancePerContainerCachingFactory(async () => {});

  expect(await factory(globalContainer)).toBe(await factory(globalContainer));
});

test("instancePerContainerCachingFactory caches the returned instance even when there is branching logic in the factory", async () => {
  const instanceA = {};
  const instanceB = {};
  let useA = true;
...
test("instancePerContainerCachingFactory returns the correct instance per container", async () => {
  const instanceA = {};
  const instanceB = {};
  let useA = true;
  ...
  expect(await factory(globalContainer)).toBe(instanceA);
  useA = false;
  expect(await factory(globalContainer.createChildContainer())).toBe(instanceB);
});
...
describe("dispose", () => {
  ...
  it("renders the container useless", async () => {
    ...
    await expect(container.resolve("indisposed")).rejects.toThrow(/disposed/);
  });

  it("disposes all child disposables", async () => {
    const container = globalContainer.createChildContainer();
  ...
  it("disposes all instances of the same type", async () => {
    const container = globalContainer.createChildContainer();

    const foo1 = await container.resolve(Foo);
    const foo2 = await container.resolve(Foo);
  ...
  it("doesn't dispose of instances created external to the container", async () => {
    const foo = new Foo();
    const container = globalContainer.createChildContainer();
